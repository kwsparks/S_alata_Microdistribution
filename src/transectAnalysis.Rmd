...
The linear model is constructed with nitrate (or squareroot(nitrte)) as the dependent variable,
transect as the first factor, and microsite as the second factor.
...
Q-Q plots were used to test for normality because the Shapiro-Wilk test is not powerful at small sample size (n=31).
Sample quantiles show noticible deviation from theoretical quantiles; the Q-Q plot suggests significant deviations from normality.
One outlier (large P value) can be seen in the Q-Q plots.
...
```{r}
soilNPKphys = read.csv(file = "/home/sparks/Desktop/S_alata_Microdistribution/Microdistribution/analysis/cleaned/soilNPKphys.csv", header = TRUE)
soilNPKphys$Transect = factor(soilNPKphys$Transect) # interpreted as factor, not numerical values
str(soilNPKphys)
```

```{r}
tissueNPKphys = read.csv(file = "/home/sparks/Desktop/S_alata_Microdistribution/Microdistribution/analysis/cleaned/tissueNPKphys.csv", header = TRUE)
...
so this analysis is preferred over the analysis of clay (untransformed).
```{r, fig.height=3, fig.width=12}
...
```{r}
tissueNPKphys$TN <- tissueNPKphys$NO3 + tissueNPKphys$NH4
```
